...
const OFFICE_KEY = 'swaramOfficeName';

function getOfficeName() {
    return localStorage.getItem(OFFICE_KEY) || '';
}

function applyOfficeName() {
    const name = getOfficeName();
    const display = name || 'Meeting';
    document.querySelector('h1').textContent = 'Swaram (സ്വരം)';
    document.querySelector('.subtitle').textContent = name ? `${name} — Auto-Transcriber` : 'Auto-Transcriber';
    const ta = document.getElementById('outputText');
    if (ta) ta.placeholder = name
        ? `${name} മീറ്റിംഗ് റിപ്പോർട്ടും തീരുമാനങ്ങളും ഇവിടെ അക്ഷരങ്ങളായി വരും...`
        : 'മീറ്റിംഗ് റിപ്പോർട്ടും തീരുമാനങ്ങളും ഇവിടെ അക്ഷരങ്ങളായി വരും...';
}
...
saveKeyBtn.addEventListener('click', () => {
    ...
    if (!officeName) {
        alert("ദയവായി Office-ന്റെ പേര് നൽകുക!");
        return;
    }
    localStorage.setItem('swaramApiKey', key);
    localStorage.setItem(OFFICE_KEY, officeName);
    apiKeyInput.value = "";
    checkApiKey();
});
...
uploadBtn.addEventListener('click', async () => {
    ...
    async function buildPromptText(extraTimeContext) {
        ...
        const officeName = getOfficeName() || 'the local government office';
...
Transcribe ONLY what is spoken. Output raw Malayalam speech as text. Identify speakers where possible using format "സ്പീക്കർ നാമം:". Do NOT add summaries, headings, formatting, decisions list, or any editorial content. Just the spoken words.${speakerReferenceNote}`;
            ...
            promptText = `Act as a professional Council Secretary for ${officeName}, Kerala. Listen to this Malayalam council meeting recording.

[TASKS]:
1. Transcribe the discussion clearly in Malayalam.
2. Fix all grammatical and speech errors to make it an official document.
3. Identify different speakers accurately when someone is addressed by name. Use format like "ചെയർപേഴ്സൺ:" or "സെക്രട്ടറി:" with their name.
4. Provide a clear, bulleted summary of the Key Decisions taken at the end.${speakerReferenceNote}`;
        }

        if (liveCouncilorData) {
            promptText += `\n\n[LIVE COUNCILOR DATA]:\nHere is the latest data from the office website containing names of current elected representatives. Use this to accurately identify speakers:\n\n${liveCouncilorData}`;
        }
